...
export class AssociatelistingComponent implements OnInit {
  ...
  FunctionDelete(associate: Associates) {
    if (confirm(`do you want to remove the associate ${associate.name} ?`)) {
      this.store.dispatch(deleteeassociate({ code: associate.id }));
    }
  }
